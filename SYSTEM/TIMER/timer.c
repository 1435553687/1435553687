...
void TIM2_IRQHandler(void)   //TIM3中断
{
	if (TIM_GetITStatus(TIM2, TIM_IT_Update) != RESET) //检查指定的TIM中断发生与否:TIM 中断源 
		...
		OLED_ShowString(0,0,(u8 *)oledBuf,16);//8*16 “温度”
		sprintf(oledBuf,"Hum:%d.%d %%",humidityH,humidityL);
		OLED_ShowString(0,16,(u8 *)oledBuf,16);//8*16 “湿度”
		sprintf(oledBuf,"Light:%.1f Lx",Light);
		OLED_ShowString(0,32,(u8 *)oledBuf,16);//8*16 “光照度”
		sprintf(oledBuf,"Somg:%.1f ppm",ppm);
		OLED_ShowString(0,48,(u8 *)oledBuf,16);//8*16 “光照度”
		OLED_Refresh();
		}
}
