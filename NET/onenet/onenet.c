...
#define AUTH_INFO	"yyb"

#define DEVID		"12345678"


extern unsigned char esp8266_buf[128];
extern u8 BeepFlag;//是否报警的标志、
extern u8 Beep_free;//控制标志
		...
		case MQTT_PKT_PUBLISH:														//接收的Publish消息
			...
			if(result == 0)
			{
				...
				else
				{
					...
					if(strstr(json_value->valuestring,"Chuan") != NULL)
					{
						json_value = cJSON_GetObjectItem(json , "value");
						if(json_value->valueint)Chuan = 0;//关窗
						else Chuan = 1;//开窗
					}
					if(strstr(json_value->valuestring,"Feng") != NULL)
					{
						json_value = cJSON_GetObjectItem(json , "value");
						if(json_value->valueint)Feng = 0;//关风扇
						else Feng = 1;//开风扇
					}
					if(strstr(json_value->valuestring,"BEEP") != NULL)
					{
						json_value = cJSON_GetObjectItem(json , "value");
						if(json_value->valueint)BeepFlag = 1;//打开蜂鸣器
						else BeepFlag = 0;//关闭蜂鸣器
						Beep_free = 0;//手动控制优先于自动控制
					}
